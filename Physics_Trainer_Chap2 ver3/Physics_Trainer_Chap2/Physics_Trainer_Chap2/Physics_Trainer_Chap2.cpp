...

{
...

    {
        ...
        printf("여기서 ( Δy = (높이) = 15 m ), ( v = 나중 속도 ), ( v0 = (처음 속도) = 22.0 m/s ), ( a(중력 가속도) = 9.80 ) 입니다.\n\n");
        printf("답을 구하기 위해서 공식을 수정하면  v^2 = v0^2 + 2 * a * Δy 입니다.\n\n");
        printf("식에 대입하면  v^2 = %6.2lf^2 + 2 * (-%6.2lf) * %6.2lf 입니다.\n\n", velocity, g, height);
        printf("이때 a 에 (-)가 붙은 이유는 돌이 떨어지는 방향이 아래이기 때문에 (-)를 곱해줘야 합니다.\n\n");
        printf("v^2을 v로 바꿔주면 식은 v = sqrt(v%6.2lf^2 + 2 * (-%6.2lf) * %6.2lf) 입니다.\n\n", velocity, g, height);
...

    {
        ...
        printf("여기서 ( v = 나중 속도 ), ( v0 = (처음 속도) = 22.0 m/s ), ( a(중력 가속도) = 9.80 ), ( t = 시간 ) 입니다.\n\n");
        printf("위로 던진 돌이%6.2lf를 지나 최고점에서 정지하고 아래로 떨어지면서 다시%6.2lf를 지나기 때문에 두 개의 식이 나옵니다.\n\n", height, height);
